...
def test_filters(token, client, Student):
    # 'between' dates
    restriction = [
        dict(attributeName="student_enroll_date", operation=">", value="2021-01-07"),
        dict(attributeName="student_enroll_date", operation="<", value="2021-01-17"),
    ]
    encoded_restriction = b64encode(dumps(restriction).encode("utf-8")).decode("utf-8")
    q = dict(
        limit=10,
        page=1,
        order="student_enroll_date DESC",
    ...
    encoded_restriction = b64encode(dumps(restriction).encode("utf-8")).decode("utf-8")
    q = dict(limit=10, page=2, order="student_id ASC", restriction=encoded_restriction)
    REST_records = client.get(
        f'/schema/{Student.database}/table/{"Student"}/record?{urlencode(q)}',
        headers=dict(Authorization=f"Bearer {token}"),
    ).json["records"]
    ...
    restriction = [
        dict(attributeName="student_name", operation="=", value="Norma Fisher"),
