...
def test_dependencies_admin(token, client, schemas_simple):
    ...
    table_a = [
        el
        for el in REST_dependencies
        if (
            el["schema"] == f"{SCHEMA_PREFIX}group1_simple" and "table_a" in el["table"]
        )
    ][0]
    ...
    table_b = [
        ...
        for el in REST_dependencies
        if (
            el["schema"] == f"{SCHEMA_PREFIX}group1_simple" and "table_b" in el["table"]
        )
    ][0]
    ...
    table_c = [
        ...
        for el in REST_dependencies
        if (
            el["schema"] == f"{SCHEMA_PREFIX}group1_simple" and "table_c" in el["table"]
        )
    ][0]
    ...
    diff_table_b = [
        ...
        for el in REST_dependencies
        if (
            el["schema"] == f"{SCHEMA_PREFIX}group2_simple"
            and "diff_table_b" in el["table"]
        )
    ][0]
